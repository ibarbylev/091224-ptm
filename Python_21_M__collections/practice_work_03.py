...
for fish_line in fish_inventory:
    name, species, tank = fish_line
    fish = {'name': name, 'species': species}
    fish_in_tanks[tank].append(fish)
...
"""Если решать эту задачу с помощью метода setdefault()"""

fish_in_tanks_2 = {}
for fish_line in fish_inventory:
    name, species, tank = fish_line
    fish = {'name': name, 'species': species}
    fish_in_tanks_2.setdefault(tank, []).append(fish)

pprint(dict(fish_in_tanks_2))
